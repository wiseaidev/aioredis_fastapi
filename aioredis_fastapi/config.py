...
class Settings:
    _redis_url: str = field(default="redis://localhost:6379", validator=validators.instance_of(str), converter=str)
    _session_id_name: str = field(default="ssid", validator=validators.instance_of(str), converter=str)
    _session_id_generator: Callable[[], str] = field(default=None, validator=validators.instance_of(Callable), converter=Callable)
    _expire_time: timedelta = field(default=timedelta(hours=6), validator=validators.instance_of(timedelta))
    _session_id: str = field(default=None, validator=validators.instance_of(str), converter=str)

    def __attrs_post_init__(self):
        def inner_session_id_generator() -> str:
            return uuid4().hex
        if not self._session_id_generator:
            object.__setattr__(self, '_session_id_generator', inner_session_id_generator)
...
@lru_cache
def settings(redis_url: Optional[str] = "redis://localhost:6379",
